...
def plot_lines() -> None:
    y1 = read_file("unaligned.txt")
    y2= read_file("aligned.txt")

    qubits = [q for q in range(4, 31)]
    plt.plot(qubits, y1, label="unaligned", lw=0.7)
    plt.plot(qubits, y2, label="aligned", lw=0.7)

    plt.xlabel("qubits")
    plt.ylabel("time (us)")
    plt.legend(fontsize="xx-small")
    plt.tight_layout(pad=0.0)
    plt.savefig("un_vs_aligned.png", dpi=600)
...
def plot_ratio() -> None:
    ...
    ratio = []
    for i, (s1, s2) in enumerate(zip(y1, y2)):
        if s1 == 0 or s2 == 0:
            continue
    ...
    aligned = [1 for r in ratio]
    print(ratio)

    plt.figure()
    plt.plot(qubits, ratio, label="unaligned", lw=0.7)
    plt.plot(qubits, aligned, label="aligned", lw=0.7)

    plt.xlabel("qubits")
    plt.ylabel("time (relative to aligned)")
    plt.legend(fontsize="xx-small")
    plt.tight_layout(pad=0.0)
    plt.savefig("un_vs_aligned_ratio.png", dpi=600)
